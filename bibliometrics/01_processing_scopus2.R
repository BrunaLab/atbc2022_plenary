...
scopus_papers <- list.files(path='./bibliometrics/scopus_files/papers',
                            full.names = TRUE)
...
write_csv(scopus_papers,"./bibliometrics/data_clean/scopus2_papers.csv")
...
scopus_authors <- list.files(path='./bibliometrics/scopus_files/authors',
                             full.names = TRUE)
...
scopus_authors
write_csv(scopus_authors,"./bibliometrics/data_clean/scopus2_authors.csv")
rm(scopus_authors2)
...
write_csv(scopus_authors_affils,"./bibliometrics/data_clean/scopus2_authors_affils.csv")
...
scopus_article_authors_wide<-scopus_authors_affils %>% 
  ...
  pivot_wider(names_from = author_count,
              ...
              names_prefix = "AF") %>% 
  unite("AF", AF1:AF72, remove = TRUE,na.rm=TRUE, sep="; ") 
# mutate(C1 = str_replace_all(C1, " \\[NA, NA, NA]" ," \\[MISSING]"))
# head(scopus_allauthors_wide$AF,40)
scopus_article_authors_wide
...
write_csv(scopus_papers_complete,"./bibliometrics/data_clean/scopus2_refs.csv")
# save csv ----------------------------------------------------------------
